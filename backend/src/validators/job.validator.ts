...
export const createJobSchema = z.object({
  ...
  location: z.string().min(2, 'Location is required').optional().nullable().or(z.literal('')),
  salary: z.string().min(1, 'Salary or Stipend details are required').optional().nullable().or(z.literal('')),
  jobType: z.nativeEnum(JobType),
  skillsRequired: z.array(z.string()).min(1, 'At least one skill is required'),
  deadline: z.string().or(z.date()).nullable().optional().transform((val) => (val ? new Date(val) : null)),
  
  responsibilities: z.string().min(5, 'Responsibilities must be at least 5 characters').optional().nullable().or(z.literal('')),
  eligibility: z.string().min(5, 'Eligibility criteria must be at least 5 characters').optional().nullable().or(z.literal('')),
