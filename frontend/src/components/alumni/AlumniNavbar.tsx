...
export default function AlumniNavbar({ onMenuToggle, onLogout, onProfileClick, onSettingsClick }: AlumniNavbarProps) {
  const { alumniProfile, loading } = useAuthContext();
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [showNotifications, setShowNotifications] = useState(false);

  const sampleNotifications = [
    { id: 1, text: 'A student viewed your profile', time: '2 hours ago' },
    { id: 2, text: 'A new mentorship request is pending', time: '5 hours ago' },
    { id: 3, text: 'Your last job posting received 12 applicants', time: '1 day ago' },
  ];
          ...
          {loading || !alumniProfile ? (
            ...
            >
              ...
              <div className="hidden md:block">
                <p className="text-xs font-semibold text-white">{alumniProfile.fullName}</p>
                <p className="text-[10px] text-slate-400 font-medium leading-none mt-0.5">Alumni</p>
              </div>
